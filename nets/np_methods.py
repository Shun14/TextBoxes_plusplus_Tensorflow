...
def ssd_bboxes_decode(feat_localizations,
    ...
    anchor_xmin, anchor_ymin, anchor_xmax, anchor_ymax = anchor_bboxes

    xref = (anchor_xmin + anchor_xmax) /2.
    yref = (anchor_ymin + anchor_ymax) /2.

    href = anchor_ymax - anchor_ymin
    wref = anchor_xmax - anchor_xmin

    decode_bbox_center_x = prior_scaling[0] * feat_localizations[:, :, 0] * wref + xref
    decode_bbox_center_y = prior_scaling[1] * feat_localizations[:, :, 1] * href + yref
    decode_bbox_width = np.exp(feat_localizations[:, :, 2] * prior_scaling[2]) * wref
    decode_bbox_height = np.exp(feat_localizations[: , :, 3] * prior_scaling[3]) * href

    xmin = decode_bbox_center_x - decode_bbox_width / 2.
    ymin = decode_bbox_center_y - decode_bbox_height / 2.
    xmax = decode_bbox_center_x + decode_bbox_width / 2.
    ymax = decode_bbox_center_y + decode_bbox_height / 2.

    x1 = prior_scaling[0] * feat_localizations[:, :, 4] * wref + anchor_xmin
    y1 = prior_scaling[1] * feat_localizations[:, :, 5] * href + anchor_ymin
    x2 = prior_scaling[0] * feat_localizations[:, :, 6] * wref + anchor_xmax
    y2 = prior_scaling[1] * feat_localizations[:, :, 7] * href + anchor_ymin

    x3 = prior_scaling[0] * feat_localizations[:, :, 8] * wref + anchor_xmax
    y3 = prior_scaling[1] * feat_localizations[:, :, 9] * href + anchor_ymax
    x4 = prior_scaling[0] * feat_localizations[:, :, 10] * wref + anchor_xmin
    y4 = prior_scaling[1] * feat_localizations[:, :, 11] * href + anchor_ymax
...
def ssd_bboxes_select_layer(predictions_layer,
    ...
    if select_threshold is None or select_threshold == 0:
        ...
        scores = np.argmax(predictions_layer, axis=2)
        mask = (classes > 0)
        classes = classes[mask]
        scores = scores[mask]
        bboxes = localizations_layer[mask]
    else:#two preditcions
...
def bboxes_clip(bbox_ref, bboxes):
    ...
    bboxes[4] = np.maximum(bboxes[4], bbox_ref[4])
    bboxes[5] = np.maximum(bboxes[5], bbox_ref[5])
    bboxes[6] = np.maximum(bboxes[6], bbox_ref[6])
    bboxes[7] = np.maximum(bboxes[7], bbox_ref[7])
    bboxes[8] = np.maximum(bboxes[8], bbox_ref[8])
    bboxes[9] = np.maximum(bboxes[9], bbox_ref[9])
    bboxes[10] = np.maximum(bboxes[10], bbox_ref[10])
    bboxes[11] = np.maximum(bboxes[11], bbox_ref[11])
